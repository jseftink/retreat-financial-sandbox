\
# retreat_financial_sandbox_v12_35.py
# -------------------------------------------------
# Version 12.35 — default inputs aligned with reference screenshot
# -------------------------------------------------
...
amber_eq = sb.number_input("Amber Equity",         0, 1_000_000,  50_000, 1_000)
jason_eq = sb.number_input("Jason Equity",         0, 1_000_000,  50_000, 1_000)

owner_loan = sb.number_input("0 % Owner Loan",     0, 1_000_000,       0, 1_000)
rate       = sb.number_input("Bank Loan Rate %",   0.0, 20.0, 7.5, 0.1)
term       = sb.number_input("Bank Loan Term (yrs)", 1, 30, 10)
...
price_camp  = sb.slider("Campsite + Dome price",            100, 1000, 300, 25)
price_cabin = sb.slider("Cabin + Dome price",               200, 1500, 575, 25)
price_prem  = sb.slider("Cabin Premium price",              300, 2000, 880, 25)
price_day   = sb.slider("Day‑Pass price",                    50,  600, 195, 5)
...
day_passes   = sb.number_input("Day‑Pass Sessions per yr", 0, 3000, 60, 10)
price_growth = sb.slider("Real Price Growth %", 0.0, 10.0, 3.8, 0.1)

sb.markdown("### Operating Costs")
wage_session = sb.number_input("Loaded wage per 90‑min session ($)", 0.0, 500.0, 78.75, 0.25)
...
st.caption(f"Version {APP_VERSION} — defaults based on 2025‑06‑30 screenshot")
...
st.caption("© 2025 Micro Retreat • v12.35 — defaults synced to screenshot")
